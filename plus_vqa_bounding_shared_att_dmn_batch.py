...
class PLUS_VQA_BOUNDING_SHARED_ATT_DMN_batch:
    ...
    def __init__(self,word2vec, word_vector_size, dim,
        ...
        bigru_sentence_input_layer=layers.InputLayer(shape=(self.batch_size,self.img_seq_len,self.dim),input_var=self.inp_c_temp)

        forward_gru_sentence=layers.recurrent.GRULayer(bigru_sentence_input_layer,num_units=self.dim)
                #ingate=gate_parameters,forgetgate=gate_parameters,cell=cell_parameters,outgate=gate_parameters)
        backward_gru_sentence=layers.recurrent.GRULayer(bigru_sentence_input_layer,num_units=self.dim,backwards=True)
                #ingate=gate_parameters,forgetgate=gate_parameters,cell=cell_parameters,outgate=gate_parameters)
        bigru_sentence_out=lasagne.layers.ElemwiseSumLayer([ forward_gru_sentence , backward_gru_sentence ])

        inp_c_bigru=layers.get_output( bigru_sentence_out)

        self.inp_c=inp_c_bigru.dimshuffle(1,2,0)
...
#######################################################################
    ...
    def process_vqa_data(self,mode,start,end):
        ...
        for i in range(self.batch_size):
            ...
            x["C"]=' . '.join(temp_Cs)
            if mode=="train":
                x["I"]=self.img_h5file_train['images_train'][ img_indices[i]-1].reshape((self.img_seq_len,self.img_vector_size))
            if mode=="test":
                x["I"]=self.img_h5file_test['images_test'][ img_indices[i]-1].reshape((self.img_seq_len,self.img_vector_size))
