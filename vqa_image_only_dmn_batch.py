...
class VQA_IMAGE_ONLY_DMN_batch:
...
#######################################################################
        ...
        net = layers.InputLayer(shape=(self.batch_size, self.dim), input_var=last_img_mem_raw)
        if self.batch_norm:
        ...
        self.params = [self.W_inp_res_in, self.W_inp_res_hid, self.b_inp_res,
                  ...
                  self.W_img_1, self.W_img_2, self.b_img_1, self.b_img_2,self.W_a]  ## Add the parameters of the Image Input Module
        ...
        if self.mode == 'train':
            ...
            self.train_fn = theano.function(inputs=[self.q_var, self.answer_var,
                                                    self.img_input_var,self.learning_rate],
                                            outputs=[self.prediction, self.loss],
                                            updates=updates)

        print "==> compiling test_fn"
        self.test_fn = theano.function(inputs=[self.q_var, self.answer_var,
                                               self.img_input_var,self.learning_rate],on_unused_input='ignore',
                                       outputs=[self.prediction, self.loss])
    ...
    def step(self,batch_index,mode):
        ...
        ret = theano_fn( q,ans,img_feat,self.lr)
