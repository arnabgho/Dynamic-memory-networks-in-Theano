...
class PLUS_VQA_BOUNDING_SHARED_ATT_DMN_batch:
    ...
    def __init__(self,word2vec, word_vector_size, dim,
        ...
        self.b_sentence_res = nn_utils.constant_param(value=0.0, shape=(self.dim,))

        self.W_sentence_upd_in = nn_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.W_sentence_upd_hid = nn_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
        self.b_sentence_upd = nn_utils.constant_param(value=0.0, shape=(self.dim,))

        self.W_sentence_hid_in = nn_utils.normal_param(std=0.1, shape=(self.dim, self.dim))
